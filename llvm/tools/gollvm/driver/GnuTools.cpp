                              ...
                              const Artifact &output)
{
  ...
  if (toolchain().driver().needsProfileLib()) {
    addProfileLib(args, cmdArgs);
  }
...
// Add instrumentation profile output libraries to link.
// Used together with instrumentation with -f[cs]profile-generate.
void Linker::addProfileLib(llvm::opt::ArgList &args,
                           llvm::opt::ArgStringList &cmdArgs) {
  std::string goinstrument = std::string("goinstrument") +
    (toolchain().driver().isPIE() ? ".pic" : "");
  std::string rtprofile = "clang_rt.profile";

  // The profile library names should be synced with
  // gollvm/staticpkg/CMakeLists.txt
  std::string libDir = toolchain().driver().installedLibDir() + "/";
  auto libName = [](const std::string &name) {
    return std::string("lib") + name + ".a";
  };
  cmdArgs.push_back(args.MakeArgString(libDir + libName(goinstrument)));
  cmdArgs.push_back(args.MakeArgString(libDir + libName(rtprofile)));
}
