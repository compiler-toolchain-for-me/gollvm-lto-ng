...
bool Driver::needsProfileLib() {
  if (args_.hasArg(gollvm::options::OPT_noprofilelib))
    return false;
  if (reconcilePath(gollvm::options::OPT_fprofile_generate,
                    gollvm::options::OPT_fprofile_generate_EQ,
                    gollvm::options::OPT_fno_profile_generate) ||
      reconcilePath(gollvm::options::OPT_fcs_profile_generate,
...
// Given options for -fXXX, -fXXX=path, -fno-XXX, select the correct value
// for the option depending on the relative position of the options on the
// command line (rightmost wins). If the winning option in not noOpt, return
